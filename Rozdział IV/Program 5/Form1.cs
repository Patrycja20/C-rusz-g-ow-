        ...
        private void Form1_KeyDown(object sender, KeyEventArgs e)
        {
           // Jeśli gracz nacisnął klawisz literki dostępnej w kontrolce ListBox, to usuwamy ją i zwiększamy tempo gry
            if (listBox1.Items.Contains(e.KeyCode))
            {
                ...
                // Gracz nacisnął prawidłowy klawisz, aktualizujemy zatem statystyki 
                // gry, wywołując metodę Update() i przekazując do niej wartość true
                stats.Update(true);
            }
            else
            {
                // Gracz nacisnął niewłaściwy klawisz, zatem aktualizujemy statystyki, 
                // przekazując w wywołaniu metody Update() wartość false 
                stats.Update(false);
            }
